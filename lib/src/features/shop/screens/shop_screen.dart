...
class _ShopScreenState extends ConsumerState<ShopScreen> {
  ...
  static const _tabs = ['All Items', 'Sphere Colors', 'Others'];
  ...
  Widget _buildItemGrid(List<ShopItem> items) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isOwned = item.isCoreColor && ownedCodes.contains(item.code);
  ...
  Widget _buildProgressBanner(List<ShopItem> items) {
    final goal = items.reduce((a, b) => a.price > b.price ? a : b);
  ...
  List<ShopItem> _filteredItems(List<ShopItem> items) {
    return switch (_selectedTab) {
      0 => items,
      1 => items.where((i) => i.isCoreColor).toList(),
      2 => items.where((i) => !i.isCoreColor).toList(),
      _ => items,
    };
  }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                '\'${item.name}\' 구매 완료! ${result.currentTotalPoints} pts 남음'),
